...
export function createRenderer (naiveComponentsDeps: Set<string>,options?:Options) {
  ...
  const overrides:Partial<Renderer> = {
      ...
      code: (code, language = 'html') => {
          ...
          const isLanguageValid = !!(hljs && language && hljs.getLanguage(language))
          const highlighted = isLanguageValid ? hljs.highlight(code, { language }).value : code
