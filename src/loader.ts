...
export async function convertMd2VueTemplateSource (text:string,options?:Options){
    ...
    const sfc = `<template><div>${md2VueTemplateSource}</div></template>\n<script setup>import {${Array.from(naiveComponentsDeps).join(',')}} from 'naive-ui'</script>`
    return sfc
}